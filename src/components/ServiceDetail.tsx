...
import { getDictionary, getServices, getService } from "@/i18n";
import { localizedHref, type Locale } from "@/i18n/config";
import { siteDetails } from "@/data/siteDetails";
import { waLink } from "@/lib/contact";

export default function ServiceDetail({ lang, slug }: { lang: Locale; slug: string }) {
  const service = getService(lang, slug);
  if (!service) notFound();
  const t = getDictionary(lang).servicePage;
  ...
  const jsonLd = {
    ...
    provider: { "@type": "Organization", name: siteDetails.siteName, url: siteDetails.siteUrl },
    url: `${siteDetails.siteUrl}${localizedHref(lang, path)}`,
    inLanguage: lang === "en" ? "en" : "id-ID",
  };

  const others = getServices(lang).filter((s) => s.slug !== service.slug);
    ...
    <>
      ...
      <main className="pt-32 pb-20 lg:pt-40 lg:pb-28">
        <div className="max-w-4xl mx-auto px-5 lg:px-8">
          <Breadcrumb
            items={[
              { name: t.breadcrumbHome, href: localizedHref(lang, "/") },
              { name: t.breadcrumbServices, href: localizedHref(lang, "/#layanan") },
              { name: service.title },
            ]}
          />
          ...
          <header className="mt-6 mb-12">
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              <a
                href={waLink(t.waService(service.title))}
                target="_blank"
                rel="noopener"
          ...
          <section className="grid lg:grid-cols-2 gap-10 mb-14">
            <div>
              <h2 className="text-2xl font-bold tracking-tight mb-5">{t.whatYouGet}</h2>
            ...
            <div>
              <h2 className="text-2xl font-bold tracking-tight mb-5">{t.whyImportant}</h2>
          ...
          <div className="rounded-3xl bg-gradient-to-br from-brand-600 to-indigo-700 text-white p-8 lg:p-12 text-center">
            ...
            <p className="mt-3 text-white/90 max-w-xl mx-auto">{t.readyDesc}</p>
            <a
              href={waLink(t.waService(service.title))}
              target="_blank"
              rel="noopener"
          ...
          <div className="mt-16">
            <h2 className="text-lg font-bold mb-5">{t.otherServices}</h2>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {others.map((o) => (
                <Link
                  key={o.slug}
                  href={localizedHref(lang, `/layanan/${o.slug}`)}
